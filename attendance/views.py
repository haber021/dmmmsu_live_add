                ...
                try:
                    ...
                    if existing_attendance:
                        ...
                        if existing_attendance.time_out:
                            messages.info(request, f"ℹ {student.name} has already checked out for {subject.code} today. Time In: {existing_attendance.time_in.strftime('%I:%M %p') if existing_attendance.time_in else 'N/A'}, Time Out: {existing_attendance.time_out.strftime('%I:%M %p')}")
                            return redirect(f"{reverse('scan')}?subject_id={subject.id}")
                        else:
                            # Student is checking out - validate time-out timing (15 minutes before class ends)
                            ...
                            if not is_timeout_valid:
                                messages.error(request, f"✗ {timeout_error}")
                                return redirect(f"{reverse('scan')}?subject_id={subject.id}")
                        ...
                        # Student is checking in (time in)
                        # Determine status based on time
                        # Use schedule time_start if available, otherwise subject's schedule_time_start, or system default
                        class_start = None
                        if schedule:
                        ...
                        if class_start:
                            ...
                            # If less than grace_period_minutes, mark as PRESENT (not late)
                        ...
                        attendance = Attendance.objects.create(
                            ...
                            subject=subject,
                            date=attendance_date,
                            time=stored_time,  # Keep for backward compatibility
                            time_in=stored_time,
                            status=status
                        )
                        messages.success(request, f"✓ Time In recorded! {student.name} - {status} at {stored_time.strftime('%I:%M %p')}")
        ...
        try:
            ...
            if existing:
                ...
                attendance = Attendance.objects.create(
                    ...
                    subject=subject,
                    date=attendance_date,
                    time=time_in,  # Keep for backward compatibility
                    time_in=time_in,
                    time_out=time_out,
                    status=status
                )
...
def attendance_logs_export_csv(request):
    ...
    for att in attendances.order_by('student__name'):
        time_in_str = '--:--'
        if att.time_in:
                        ...
                        if not is_valid:
                            ...
                            if existing:
                                ...
                                if existing.time_out:
                                    time_in_str = existing.time_in.strftime('%I:%M %p') if existing.time_in else 'N/A'
                                    time_out_str = existing.time_out.strftime('%I:%M %p')
                                    messages.info(request, f"ℹ {student.name} already checked out. Time In: {time_in_str}, Time Out: {time_out_str}")
                                else:
                                    # Student is checking out - validate time-out timing (15 minutes before class ends)
                                    is_timeout_valid, timeout_error = validate_timeout_time(
